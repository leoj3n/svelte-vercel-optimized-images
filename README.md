...
- Edit `./src/routes/+page.svelte` and add an `img` tag pointing to `/cat1.jpeg` as well as import `./styles.css`:
  ```html
  <script>
    import './styles.css';
  </script>
...
Vercel will only generate optimized images if configured to do so in `.vercel/output/config.json` (generated during `vite build`).

We will add a new build command that calls a custom script that modifies the Vercel configuration JSON post build.
