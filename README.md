...
Vercel will only generate optimized images if configured to do so in `.vercel/output/config.json`. For this we will add a new build command that calls a custom script to modify the Vercel configuration JSON that is generated by `vite build`.
